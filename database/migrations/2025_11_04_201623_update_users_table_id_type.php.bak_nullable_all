...
class UpdateUsersTableIdType extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        if (Schema::hasTable('users')) {
            // D'abord, vérifier si la colonne id existe et la modifier
            DB::statement('ALTER TABLE `users` MODIFY `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT;');
        }

        // Liste des tables avec des colonnes qui référencent users.id
        $tables = [
            'sections' => ['teacher_id'],
        ...
        // Mettre à jour le type des colonnes qui référencent users.id
